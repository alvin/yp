...
<svelte:head><title>Deposits received · {dateShort(data.date)}</title></svelte:head>

<ReportShell
	title="Deposits Received — {dateLong(data.date)}"
	backHref="/reports/dcar?date={data.date}"
	backLabel="Daily Cash"
	tabs={dailyCashTabs(data.date, 'deposits-received')}
	date={data.date}
>
	{#snippet toolbar()}
		{#if agrees}
		...
		{:else}
			<Badge variant="warning">Differs by {money(Math.abs(total - data.dcar_line))}</Badge>
		{/if}
	{/snippet}
	<div class="blackbar">Yellow Point Lodge</div>
	<h1>Deposits Received</h1>
	...
	<table>
		...
		<tbody>
			...
			{:else}
				<tr><td colspan="7" class="small">No deposits received on this date.</td></tr>
			{/each}
		</tbody>
	</table>
	<p class="right">
		<b>Total Deposits Received:</b>&nbsp;&nbsp; <b>{money(total)}</b>
	</p>
	<ReportFooter />
</ReportShell>
